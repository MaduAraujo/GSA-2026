...
export const ChallengesModule: React.FC<ChallengesModuleProps> = ({
  ...
  const addDateToForm = (date: string) => {
    if (!date) return;
    setFormData((prev) => {
      const existing = prev.dates || [];
      if (existing.includes(date)) return prev;
      return { ...prev, dates: [...existing, date].sort() };
    });
  };

  const handleRemoveDate = (date: string) => {
    setFormData((prev) => ({ ...prev, dates: (prev.dates || []).filter((d) => d !== date) }));
  };
  ...
  const filteredChallenges = challenges
    ...
    .sort((a, b) => {
      const aDate = a.dates?.[0] || a.deadline;
      const bDate = b.dates?.[0] || b.deadline;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dates = (formData.dates || []).slice().sort();
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500 pt-1">
                  {(challenge.dates && challenge.dates.length > 0
                    ? challenge.dates
                    : challenge.deadline
                    ? [challenge.deadline]
                    : []
                    ...
                    <span className="inline-flex items-center gap-1">
                      ...
                      {formatDateBR((challenge.dates && challenge.dates[0]) || challenge.deadline)}
                      {challenge.dates && challenge.dates.length > 1 ? ` +${challenge.dates.length - 1} dia${challenge.dates.length - 1 > 1 ? 's' : ''}` : ''}
                    </span>
                  )}
                  ...
                  {formData.dates && formData.dates.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {formData.dates.map((date) => (
                        <span
                          key={date}
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium bg-[#34A853]/10 text-[#1E8E3E] border border-[#34A853]/20"
                        >
