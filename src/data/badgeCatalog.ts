...
export const BADGE_CATALOG: BadgeDefinition[] = [
  ...
  {
    id: 'twenty_certificates',
    name: 'Lenda dos Certificados',
    description: 'Acumulou 20 certificados no seu portfólio.',
    hint: 'Envie 20 certificados',
    icon: Diamond,
    tier: 'gold',
    isMet: (s) => s.certificateCount >= 20,
    progress: (s) => ({ current: Math.min(s.certificateCount, 20), target: 20 }),
  },
  {
    id: 'seven_categories',
    name: 'Generalista',
    description: 'Tem certificados em 7 categorias diferentes.',
    hint: 'Diversifique em 7 categorias',
    icon: Compass,
    tier: 'gold',
    isMet: (s) => s.categoryCount >= 7,
    progress: (s) => ({ current: Math.min(s.categoryCount, 7), target: 7 }),
  },
  {
    id: 'twenty_skills',
    name: 'Enciclopédia Viva',
    description: 'Cadastrou 20 habilidades únicas em seus certificados.',
    hint: 'Cadastre 20 skills únicas',
    icon: TrendingUp,
    tier: 'gold',
    isMet: (s) => s.uniqueSkillCount >= 20,
    progress: (s) => ({ current: Math.min(s.uniqueSkillCount, 20), target: 20 }),
  },
  {
    id: 'first_favorite_prompt',
    name: 'Prompt Favorito',
    description: 'Favoritou seu primeiro prompt no vault.',
    hint: 'Favorite 1 prompt',
    icon: Heart,
    tier: 'bronze',
    isMet: (s) => s.favoritePromptCount >= 1,
    progress: (s) => ({ current: Math.min(s.favoritePromptCount, 1), target: 1 }),
  },
  {
    id: 'twenty_prompts',
    name: 'Arquiteto de Prompts',
    description: 'Construiu uma biblioteca com 20 prompts salvos.',
    hint: 'Salve 20 prompts',
    icon: Library,
    tier: 'gold',
    isMet: (s) => s.promptCount >= 20,
    progress: (s) => ({ current: Math.min(s.promptCount, 20), target: 20 }),
  },
  {
    id: 'three_posts',
    name: 'Criador Consistente',
    description: 'Criou 3 posts com o Gemini Posts.',
    hint: 'Crie 3 posts',
    icon: CalendarCheck,
    tier: 'silver',
    isMet: (s) => s.postCount >= 3,
    progress: (s) => ({ current: Math.min(s.postCount, 3), target: 3 }),
  },
  {
    id: 'ten_published_posts',
    name: 'Referência Digital',
    description: 'Publicou 10 posts nas redes sociais.',
    hint: 'Publique 10 posts',
    icon: ThumbsUp,
    tier: 'gold',
    isMet: (s) => s.publishedPostCount >= 10,
    progress: (s) => ({ current: Math.min(s.publishedPostCount, 10), target: 10 }),
  },
  {
    id: 'all_star',
